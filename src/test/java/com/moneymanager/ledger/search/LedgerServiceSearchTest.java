...
class LedgerServiceSearchTest {
	...
	void getBudgetBooksForSummary_memoMode_returnResponse() {
		...
		LedgerListResponse.DayCards mockDayCards = LedgerListResponse.DayCards.builder()
				...
				.cardList(List.of(
						LedgerListResponse.Card.builder()
								.id(1L)
								.category(CategoryResponse.builder().code("010101").name("식비").build())
								.price(20000L)
								.memo("점심")
								.build()
				))
		...
		when(budgetBookDao.findLedgersBySearch(memberId, search.getMode(), search.getKeywords(), DateScope.ofYearMonth(search.getYear(), search.getMonth())))
				.thenReturn(List.of(mockDayCards));

		//when
		LedgerListResponse response = ledgerService.getLedgersForSummary(memberId, search);
