...
import com.moneymanager.domain.budgetBook.vo.SearchPeriod;
 ...
 * 파일이름       : BudgetBookDaoSearchTest<br>
 * 작성자          : areum Jang<br>
 * 생성날짜       : 25. 11. 24.<br>
 * 설명              : BudgetBookDao 기능을 검증하는 테스트 클래스
 * </p>
 * <br>
...
class BudgetBookDaoSearchTest {
	...
	@BeforeEach
	void setUp() {
		this.budgetBookDao = new BudgetBookDao(jdbcTemplate);
	}
	...
	void 월단위_메모검색_목록반환(){
		...
		LedgerListResponse.DayCards expected = LedgerListResponse.DayCards.builder()
				...
				.cardList(List.of(
						LedgerListResponse.Card.builder()
								.id(1L)
								.category(LedgerCategoryResponse.builder().code("0201010").name("식비").build())
								.price(15000L)
								.memo("점심")
								.build()
				))
		...
		//when
		List<LedgerListResponse.DayCards> result = budgetBookDao.findBudgetBooksBySearch(
				"member",
				"memo",
				List.of("점심"),
