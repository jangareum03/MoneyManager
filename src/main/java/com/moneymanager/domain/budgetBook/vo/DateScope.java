...
public class DateScope implements DateGroupable {
	...
	private static void validateMonth(int month) {
		if( !( 0 < month && month < 13) ) {
			throw createClientException(ErrorCode.BUDGET_DATE_INVALID, "월은 1~12까지만 가능합니다.", month);
		}
	}
	...
	private static void validateWeek(int week) {
		if( !(0 < week && week < 7) ) {
			throw createClientException(ErrorCode.BUDGET_DATE_INVALID, "주는 1~6까지만 가능합니다.", week);
		}
	}
