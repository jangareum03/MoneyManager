...
public class CategoryService {
	...
	public List<CategoryResponse> getTopCategories() {
		List<CategoryResponse> readList = new ArrayList<>();

		for ( Category category : categoryDAO.findCategory() ) {
			readList.add(CategoryResponse.builder().name(category.getName()).code(category.getCode()).build());
		}
	 ...
	 * @return 하위 카테고리 리스트
	 */
	public List<CategoryResponse> getMySubCategories(String code) {
		List<CategoryResponse> categoryList = new ArrayList<>();

		for (Category entity : categoryDAO.findCategoryByCode(code)) {
			categoryList.add(CategoryResponse.builder().name(entity.getName()).code(entity.getCode()).build());
		}
