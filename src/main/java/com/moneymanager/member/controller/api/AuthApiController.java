...
public class AuthApiController {
    ...
    public ApiBody<Void> sendCode(@RequestBody SendVerificationCodeRequest request) {
        ...
        return ApiBody.<Void>builder()
                .messageKey("email.verification.send")
                .build();
    }

    @PostMapping("/email/verify-code")
    @Operation(ServiceAction.MEMBER_EMAIL_CHECK)
    public ApiBody<Void> verifyEmail(@RequestBody EmailVerifyRequest request, HttpServletResponse response) {
        emailVerificationService.verifyCode(request.email(), request.code());

        //쿠키 설정
        ResponseCookie cookie = emailVerificationService.saveCookie(
                "emailVerifyToken",
                emailVerificationService.issueEmailToken(request.email()),
                "/"
        );
    ...
    public ApiBody<Void> verifyRefreshToken(@CookieValue("refreshToken") String refreshToken, HttpServletResponse response) {
        ...
        return ApiBody.<Void>builder().build();
    }
