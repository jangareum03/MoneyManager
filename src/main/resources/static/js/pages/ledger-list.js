...
function initButton() {
    //검색 메뉴인 경우
    const menus = document.querySelectorAll('.ledger-menu__item');
    menus.forEach( menu => {
...
function handlerClick( event ) {
    ...
    if( target.classList.contains('icon--calendar') ) {
        container = document.querySelector('.ledger-date');
    ...
    //메뉴를 클릭할 때
    if( target.closest('.ledger-menu__filter') ) {
        container = document.querySelector('.ledger-menu');
        ...
        //기존 검색메뉴 삭제 후 생성
        container.querySelector('.ledger-menu__bottom')?.remove();
        const searchBox = document.createElement('section');
        searchBox.classList.add('ledger-menu__bottom');
    ...
    //검색 버튼을 클릭할 때
    container = target.closest('.ledger-menu__bottom');
    if( container ) {
        //메모 검색할 경우
    ...
    //가계부 카드를 클릭할 때
    if( target.closest('.ledger-list__cards') ) {
        if( !target.querySelector('.empty') ) {
            const isDelMode = document.querySelector('.ledger-menu__action--delete');

            if( !isDelMode ) {
                const id = target.querySelector('.ledger-card__icon').dataset.pk;
                goToPage(`/ledgers/${id}`);
            }
        }
    }
    ...
    if( target.classList.contains('button--delete') ) {
        container = target.closest('.ledger-menu__action');

        const isDelMode = container.classList.contains('ledger-menu__action--delete');
        container.classList.toggle( 'ledger-menu__action--delete', !isDelMode );
    ...
    //삭제 체크박스를 클릭할 때
    container = target.closest('.ledger-card');
    if( container ) {
...
async function handlerChange( event ) {
    ...
    //날짜 선택박스를 변경할 때
    container = target.closest('.ledger-date');
    if( container ) {
        //현재 날짜 범위 알기
        const dateType = container.dataset.type;
        let url = `/ledgers/list/${dateType}`;
    ...
    //검색 영역 선택박스를 변경할 때
    container = document.querySelector('.ledger-menu__bottom');
    if( container ) {
        //최상위 카테고리 변경할 때
...
function truncateMemos() {
    const cards = document.querySelectorAll('.ledger-card');
    cards.forEach( card => {
        const memo = card.querySelector('.ledger-card__memo');
...
function loadGoogleChart() {
    const title = document.querySelector('.ledger-date');
...
function drawChart(chartData) {
    ...
    let chart = new google.visualization.ColumnChart(document.querySelector('.ledger-graph__content'));
...
function updateActiveTab( container, mode ) {
    const tabs = container.querySelectorAll('.ledger-menu__item');

    tabs.forEach( tab => {
        const isActive = tab.dataset.mode === mode;
        tab.classList.toggle( 'ledger-menu__item--active', isActive );
    });
}
...
async function renderInOutCategories( container, mode ) {
    ...
    //라디오 박스 생성
    const radioBox = createDiv({ parent: container, classList: ['ledger-menu__search'] });
...
async function renderSearchResults( mode ) {
    ...
    console.log(data);
    const {price, info} = await fetchLedgerSearch( data );
...
function getKeywords( mode ) {
    ...
    const container = document.querySelector('.ledger-menu__bottom');
    switch( mode ) {
        case 'inout' :
...
function updateSummaryStats( price ) {
    const container = document.querySelector('.ledger-stats');
    if( !container ) return;

    //전체금액
    container.querySelector('.ledger-stats__total dd').textContent = price.total ? formatNumber(price.total) + '원' : '0원';

    //수입금액
    container.querySelector('.ledger-stats__income dd').textContent = price.income ? formatNumber(price.income) + '원' : '0원';

    //지출금액
    container.querySelector('.ledger-stats__outlay dd').textContent = price.outlay ? formatNumber(price.outlay) + '원' : '0원';
}
...
function renderFilledResult( container, result ) {
    Object.entries(result).forEach( ([key, cards]) => {
        ...
        title.classList.add('ledger-list__title');
        ...
        for( let i=0; i < cards.length; i += 3 ) {
            ...
            ul.classList.add('ledger-list__cards');
...
function createEmptyCard() {
    ...
    card.classList.add('ledger-card');

    //아이콘 생성
    createDiv({ parent: card, classList: ['ledger-card__type'] });

    //카테고리
    createSpan({ parent: card, classList: ['ledger-card__category'] });

    //금액
    createSpan({ parent: card, classList: ['ledger-card__price'] });

    //메모
    createP({ parent: card, classList: ['ledger-card__memo'] });
...
function createCard( item ) {
    ...
    //삭제 아이콘 추가
    const iconBox = createDiv({ parent: card, classList: ['ledger-card__icon'] });
    ...
    card.querySelector('.ledger-card__category').textContent = item.name;
    card.querySelector('.ledger-card__price').textContent = formatNumber(item.price) + '원';
    card.querySelector('.ledger-card__memo').textContent = changeMemo( item.memo, MAX_MEMO );
...
function toggleDeleteMode( on, container ) {
    const ledgerBox = document.querySelector('.ledger-list');

    if( on ) {
        activateDeleteMode( container, ledgerBox );
    }else {
        deactivateDeleteMode( container, ledgerBox );
    }
}
...
function activateDeleteMode( container, box ) {
    box.classList.add('ledger-list--delete');

    box .querySelectorAll('.ledger-card:not(.empty)').forEach( (card) => {
        card.addEventListener( 'click', handlerClick );
    });
...
function deactivateDeleteMode( container, box ) {
    box.classList.remove('ledger-list--delete');
    container.classList.remove('ledger-menu__action--delete');

    // 삭제 문구 & 취소 버튼 제거
    container.querySelector('.delete__count')?.remove();
    container.querySelector('.ledger-menu__action .button--cancel')?.remove();
...
async function confirmAndDelete() {
    ...
    const apiData = await fetchLedgerDelete( selectedBoxes );
...
function decorateSelect( container, select ) {
    const label = createLabel({ classList: ['ledger-select__label'] });
    label.appendChild(select);

    const span = createSpan( {parent:label, classList: ['ledger-select__icon'] } );
